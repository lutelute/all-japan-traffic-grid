...
## 長期目標: 学習ベースの軽量6500万トリップシミュレーション

### ビジョン
- 日本全国6500万トリップ/日をリアルに再現する
- MATSimをそのまま6500万体回すのは非現実的（2TB+ RAM必要）
- **部分学習 + 空間分割並列** で軽量に実現する

### アプローチ案
1. **空間分割学習**: 日本を20-50のゾーンに分割し、各ゾーンで独立にMATSimイテレーションを回す
2. **境界条件の学習**: ゾーン間の流動パターンを学習し、境界需要として注入（現在のpartitioned.pyの2パスを拡張）
3. **収束ループ**: ゾーン内学習 → 境界交換 → 再学習 を収束まで繰り返す
4. **軽量推論**: 学習済みのルート選択モデル（NN/GBM等）でMATSimなしに高速推論
5. **並列実行**: 各ゾーンを独立プロセス/マシンで並列実行

### 段階的スケールアップ
| フェーズ | エージェント | 手法 | 環境 |
|---|---|---|---|
| 現在 | 10万 | MATSim単体 | ローカル(36GB) |
| 次 | 50万 | MATSim + cloud | AWS r6g.4xlarge |
| 中期 | 500万 | 分割学習並列 | AWS x N台 |
| 最終 | 6500万 | 学習済み軽量推論 | ローカルでも可 |

### 技術的な課題
- 部分学習の収束保証（ゾーン間の相互依存をどう扱うか）
- ルート選択モデルの汎化性能
- パーソントリップ調査データの統合（ODマトリクスの現実性向上）
